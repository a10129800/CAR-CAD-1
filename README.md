...
* **📐 專業 CAD 工程三視圖與四分屏系統 (Orthographic 3-View & Quad-View)**：
  * **正交無形變投影**：採用真正的 `OrthographicCamera`，平行線永不交會，符合工業製圖標準。
  * **四分割並列視窗**：
    * **左上（俯視圖 TOP VIEW）**：正上方垂直透視，展示流暢雙曲面空力廓形與輪拱寬體（標注全長 158mm × 全寬 90mm）。
    * **左下（前視圖 FRONT VIEW）**：車頭正向投影，檢視星環 LED 光刃、前下巴定風翼與離地間隙（標注全寬 90mm × 全高 46mm）。
    * **右下（側視圖 SIDE VIEW）**：左側面正交投影，展示超跑極致低風阻輪廓與前後軸距（標注前後軸距 96mm、輪徑 Ø36mm）。
    * **右上（3D 等角透視 ISOMETRIC）**：保留 360° 自由軌道旋轉、高光金屬漆面、車底霓虹燈光效與空氣套件分解。
  * **同步等比縮放**：在正交視區內滾動滑鼠滾輪，三視圖等比例同步縮放，保持 1:1 視圖對齊。
  * **視窗單獨全螢幕**：點擊任一視窗右上角 `⛶`，即時全螢幕深入檢視該正交視角細節。
  * **即時尺寸標注 (CAD Dimensions)**：疊加顯示全長、全寬、全高、軸距、輪距與離地間隙標注線，支援一鍵開關。
  * **📸 一鍵匯出 CAD 工程圖紙 (2.5K Ultra HD PNG)**：
    * **多處便捷按鈕**：頂部 HUD 資訊列、四分屏中心徽章、右上 3D 視窗標題列、正交提示列與側邊面板皆提供快捷按鈕。
    * **超高解析度離屏合成 (2400×1500)**：自動多視區渲染正俯視圖 (1:1)、正前視圖 (1:1)、正側視圖 (1:1) 與 3D 等角透視圖。
    * **標準工程圖紙規範**：包含 ISO/JIS 標準圖框、區域座標刻度 (1~8 / A~F)、高精度尺寸標注引線、規格參數表與第三角投影法標記 (Third Angle Projection Title Block)。
